...
function checkFile(filePath) {
  if (filePath.endsWith('.vue') || filePath.endsWith('.ts') || filePath.endsWith('.js')) {
    ...
    if (consoleMatches && consoleMatches.length > 0) {
      warnings.push(
        `Console statements found in ${filePath.replace(srcPath, 'src')}: ${consoleMatches.length} instances`,
      )
    }
    ...
    if (alertMatches && alertMatches.length > 0) {
      errors.push(
        `Alert statements found in ${filePath.replace(srcPath, 'src')}: ${alertMatches.length} instances`,
      )
    }
    ...
    if (todoMatches && todoMatches.length > 0) {
      warnings.push(
        `TODO/FIXME comments found in ${filePath.replace(srcPath, 'src')}: ${todoMatches.length} instances`,
      )
    }
  }
}
...
const demoFiles = ['db.json', 'mock-data.json', 'test-data.json']
