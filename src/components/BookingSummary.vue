  ...
  <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-soft border border-neutral-200 dark:border-neutral-700 p-6">
    ...
    <div class="mb-6">
      <div class="flex space-x-4">
        ...
        />
        <div class="flex-1">
          <h4 class="font-semibold text-neutral-800 dark:text-neutral-200">{{ booking.hotel.name }}</h4>
          ...
          <div class="flex items-center mt-1">
            ...
            <span class="text-sm text-neutral-600 dark:text-neutral-400 ml-1">{{ booking.hotel.rating }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-4 mb-6">
      ...
      <div class="flex justify-between items-center py-2 border-b border-neutral-100 dark:border-neutral-700">
        <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ $t('booking.dates') }}</span>
      ...
      <!-- Duration -->
      <div class="flex justify-between items-center py-2 border-b border-neutral-100 dark:border-neutral-700">
        <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ $t('booking.duration') }}</span>
        <span class="text-sm font-medium text-neutral-800 dark:text-neutral-200">
      ...
      <!-- Room Type -->
      <div v-if="booking.room" class="flex justify-between items-center py-2 border-b border-neutral-100 dark:border-neutral-700">
        <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ $t('booking.roomType') }}</span>
        <span class="text-sm font-medium text-neutral-800 dark:text-neutral-200">{{ booking.room.type }}</span>
      </div>

      <!-- Guests -->
      <div class="flex justify-between items-center py-2 border-b border-neutral-100 dark:border-neutral-700">
        <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ $t('booking.guests') }}</span>
        <span class="text-sm font-medium text-neutral-800 dark:text-neutral-200">
          {{ booking.guests }} {{ booking.guests === 1 ? $t('booking.guest') : $t('booking.guests') }}
        </span>
      </div>

      <!-- Rooms -->
      <div class="flex justify-between items-center py-2 border-b border-neutral-100 dark:border-neutral-700">
        <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ $t('booking.rooms') }}</span>
        <span class="text-sm font-medium text-neutral-800 dark:text-neutral-200">
          {{ booking.rooms || 1 }} {{ (booking.rooms || 1) === 1 ? $t('booking.room') : $t('booking.rooms') }}
        </span>
      </div>
    </div>
    ...
    <div v-if="showDetails" class="space-y-3 mb-6">
      <h4 class="font-semibold text-neutral-800 dark:text-neutral-200 mb-3">{{ $t('booking.priceBreakdown') }}</h4>
      
      <!-- Room Rate -->
      <div class="flex justify-between text-sm">
        <span class="text-neutral-600 dark:text-neutral-400">
          ${{ roomRate }} × {{ nights }} {{ nights === 1 ? $t('booking.night') : $t('booking.nights') }} × {{ booking.rooms || 1 }} {{ (booking.rooms || 1) === 1 ? $t('booking.room') : $t('booking.rooms') }}
        </span>
      ...
      <div class="border-t border-neutral-200 dark:border-neutral-700 pt-3 flex justify-between font-semibold">
        ...
        <span class="text-xl text-primary-600 dark:text-primary-400">${{ booking.finalPrice }}</span>
      </div>
    </div>
    ...
    <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 mb-6">
      <div class="flex items-start space-x-2">
        ...
        <div class="text-sm">
          <div class="font-medium text-green-800 dark:text-green-200 mb-1">{{ $t('booking.freeCancellation') }}</div>
          <div class="text-green-700 dark:text-green-300">{{ $t('booking.cancellationPolicy') }}</div>
        </div>
      </div>
    </div>

    <!-- Guest Info Summary (for booking review) -->
    <div v-if="showGuestInfo && booking.guestInfo?.firstName" class="border-t border-neutral-200 dark:border-neutral-700 pt-6">
      <h4 class="font-semibold text-neutral-800 dark:text-neutral-200 mb-3">{{ $t('booking.guestInformation') }}</h4>
    ...
    <!-- Support Notice -->
    <div class="text-xs text-neutral-500 dark:text-neutral-400 text-center mt-6 pt-4 border-t border-neutral-200 dark:border-neutral-700">
