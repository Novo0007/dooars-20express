<template>
  <div
    class="fixed bottom-4 right-4 bg-white dark:bg-neutral-800 rounded-lg shadow-lg p-4 text-xs max-w-sm z-50"
    v-if="showDebug"
  >
    ...
    <div class="space-y-2 text-neutral-600 dark:text-neutral-400">
      ...
      <div v-if="authStore.user"><strong>User:</strong> {{ authStore.user.email }}</div>

      <div v-if="authStore.user?.profile">
        <strong>Profile:</strong> {{ authStore.user.profile.full_name }} ({{
          authStore.user.profile.role
        }})
      </div>

      <div v-if="authStore.session"><strong>Session:</strong> Valid</div>
      ...
      <div><strong>Loading:</strong> {{ authStore.loading ? 'Yes' : 'No' }}</div>
    </div>
  </div>
