...
export function testErrorSerialization() {
  ...
  const serializedError = mockSupabaseError instanceof Error ? mockSupabaseError.message : 'Unknown error'
  ...
  const notificationMessage = testError instanceof Error ? testError.message : 'Failed to process request'
...
export function simulateRoomsTableError() {
  ...
  const errorMessage = typeof mockRoomsError === 'object' && mockRoomsError.message 
...
export function runAllErrorTests() {
  ...
  const allPassed = Object.values(serializationTest).every(Boolean) && 
                   roomsTableTest && 
                   featuredHotelsTest
